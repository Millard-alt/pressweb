...
if (DATABASE_URL.length > 0) {
  ...
  // A dead idle client must never take the whole process down.
...
/* Tables whose id is an auto-incrementing BIGSERIAL. Inserts get `RETURNING id`
   appended so routes keep reporting `lastInsertRowid` exactly like SQLite. */
const AUTO_ID_TABLES = new Set(['users', 'articles', 'team', 'calendar', 'categories']);
...
async function seedIfEmpty() {
  ...
  const settingsCount = (await get('SELECT COUNT(*) AS n FROM settings')).n;
  if (settingsCount === 0) {
    const rows = [
    ...
    for (const [k, v] of rows) await run('INSERT INTO settings(key, value) VALUES (?,?)', [k, v]);
  }
}
...
module.exports = {
  db, ready, DEFAULT_CATEGORIES, CATEGORIES: DEFAULT_CATEGORIES,
  get backend() { return backend; },
  get usingPostgres() { return backend === 'postgres'; }
};
